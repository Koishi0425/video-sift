...
- [ ] 重构 GUI 为左侧导航栏 + 主内容区结构。
- [ ] 设置主分区：「处理任务」「任务记录」「设置」「关于」。
- [ ] 将下载、转写、总结作为任务阶段展示，而不是独立导航分区。
- [ ] 将日志作为当前任务的可展开详情展示，默认折叠，支持清空和复制。

### P1 处理任务

- [ ] 优化任务输入区域，清晰支持 URL、BV 号和本地音视频文件。
- [ ] 增加明确的拖拽区域和文件选择入口。
- [ ] 保留处理模式选择：完整处理、仅下载音频、仅转写、仅重新总结。
- [ ] 优化语言、Whisper 模型等参数选择。
- [ ] 增加任务开始、取消、运行中状态展示。
- [ ] 默认仅展示当前阶段、整体进度条和简短状态说明。
- [ ] 将详细日志折叠展示，用户可手动展开查看完整输出。
- [ ] 任务失败时自动提示关键错误，并提供展开日志入口。
- [ ] 对下载失败、配置缺失、依赖缺失给出可操作提示。

### P2 任务记录与结果阅读

- [ ] 增加历史任务列表，展示来源、标题、模型、语言、状态和输出路径。
- [ ] 增加结果阅读区，直接渲染 `summary.md`，让用户无需打开外部编辑器即可阅读总结。
- [ ] 支持切换查看「总结」「转写文本」「带时间戳转写」「日志」。
- [ ] 增加结果操作：复制总结、打开文件、打开任务目录、重新总结。
- [ ] 优化 Markdown 阅读样式，提升标题、段落、列表和代码块可读性。

### P3 设置与发行

- [ ] 增加设置页面，管理 DeepSeek API、代理、cookies、默认模型、默认语言。
- [ ] 增加依赖检查，提示 ffmpeg、yt-dlp、Whisper 模型缓存等状态。
