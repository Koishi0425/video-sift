...
# Whisper 初始提示词，用于引导模型输出标点符号
WHISPER_INITIAL_PROMPTS = {
    "zh": "以下是普通话说出的句子。请使用正确的标点符号，包括逗号、句号、问号、感叹号等。",
    "en": "The following is a conversation in English. Please use proper punctuation including commas and periods.",
    "ja": "以下は日本語の会話です。適切な句読点を含めてください。",
    "ko": "다음은 한국어 대화입니다. 적절한 구두점을 포함하세요.",
}
...
def transcribe_audio(audio_path: Path, workdir: Path, detect_model_name: str, whisper_model: str, language: str) -> tuple[str, str | None]:
    ...
    for index, (chunk_path, start_ms) in enumerate(chunks, start=1):
        ...
        transcribe_kwargs = dict(language=transcribe_language, fp16=False)
        initial_prompt = WHISPER_INITIAL_PROMPTS.get(transcribe_language)
        if initial_prompt:
            transcribe_kwargs["initial_prompt"] = initial_prompt
            logging.info("使用初始提示词引导标点符号输出")
